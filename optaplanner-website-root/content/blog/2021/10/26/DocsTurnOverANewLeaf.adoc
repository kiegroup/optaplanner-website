...
This now changes with launching a new documentation website, built using https://antora.org/[Antora].

What's so exciting about the new documentation?

First, it does not take ages to load, compared to single-page HTML documentation.
For example, if I want to read about _repeated planning_,
I open the single-page HTML docs and wait for 25 seconds, despite having a very good cable connection, for the page to load.
With the new documentation, it took me only 2 seconds, as each chapter has its own HTML page, and thus,
the content that has to load is limited.
