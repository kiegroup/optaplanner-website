...
The new methods behave differently when it comes to nullable planning variables.
The deprecated methods will eventually be removed in OptaPlanner 9.

====  Impact on users without nullable planning variables

Users who do not use over-constrained planning (where `@PlanningVariable(..., nullable = false)`) may simply replace all the uses of the deprecated methods.
...
Users who use over-constrained planning (where `@PlanningVariable(..., nullable = true)`) need to take into account the following:

* `forEach(...)` will filter out planning entities with `null` values in any of their genuine planning variables,
...
you may remove these null checks after replacing `from(...)` with `forEach(...)`.
...
you have to replace `from(...)` with `forEachIncludingNullVars(...)`.
...
----
    ...
    Constraint femaleInMaleRoomConstraint(ConstraintFactory constraintFactory) {
        return constraintFactory.from(BedDesignation.class)
