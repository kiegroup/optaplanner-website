...
:awestruct-tags: [insight, benchmark, algorithm]
...
(in part because of http://www.optaplanner.org/blog/2014/03/27/IsTheSearchSpaceOfAnOptimizationProblemReallyThatBig.html[the size of the search space]).
...
* If local optima are the problem (middle pattern), try adding course grained moves
...
Instead, let the http://www.optaplanner.org[OptaPlanner] Benchmarker guide you
