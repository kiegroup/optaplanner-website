...
In it, we see the constraint "Assign every shift" has 1116 matches, meaning we have
1116 unassigned shifts. Let's fix that right now.

image::ConstraintSummary1.png[Unassigned Shifts]

Close the summary popup and click on the "Solve" button. OptaPlanner will begin solving
...
making sure no employee get two shifts at the same time, each employee has the required
...
circle mean the employee has a low Covid-19 susceptibility, while a more filled in
circle mean a higher Covid-19 susceptibility. Finally, a red circle mean the employee
is extremely susceptible to Covid-19, and should not placed in a ward with Covid-19.
...
as such is not able to make their shift this Saturday. Luckily for us, OptaPlanner
...
in https://developers.redhat.com/devnation/tech-talks/AI-vs-covid19-java/[Geoffery De Smet's talk on the application].