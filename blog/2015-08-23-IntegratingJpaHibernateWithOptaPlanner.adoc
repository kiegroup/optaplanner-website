...
with an OptaPlanner entity (an object that gets changed by OptaPlanner during solving).

== Persisting a score

By default, JPA Hibernate will put a `Score` in a `BLOB` column through Java serialization.
This is undesirable because it prevents using the score in an JPA-QL query.
Furthermore, it triggers database issues when upgrading the OptaPlanner version.

Therefore, OptaPlanner `6.4.0.Beta1` has a new jar `optaplanner-persistence-jpa` to contains a Hibernate type for each score type.
Use it like this:
...
The docs contain more information on how to deal with `BigDecimal` and/or bendable scores properly.
