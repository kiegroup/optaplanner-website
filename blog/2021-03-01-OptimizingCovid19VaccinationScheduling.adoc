...
:awestruct-author: pbrown and emmurphy
//not sure if can have two authors. If not, we'll make it Paul.
:awestruct-layout: blogPostBase
:awestruct-tags:


COVID-19 vaccination appointment scheduling has proven to be a world-wide challenge. People eligible for vaccinations haven’t been able to secure appointments despite repeated attempts. Those scheduled for vaccinations sometimes arrive at a vaccination center only to learn that their appointment has been cancelled. Others find that they share the same vaccination time window with hundreds of people and must wait in line for hours. However, this doesn’t have to be the case. You can use the OptaPlanner vaccination appointment scheduler quickstart to develop a schedule that is both efficient and fair. The vaccination appointment scheduler uses artificial intelligence (AI) to prioritize people and allocate time slots based on multiple constraints and priorities.  It is part of the https://github.com/kiegroup/optaplanner-quickstarts[OptaPlanner quickstart collection], available on GitHub.
...
One solution is to equally share appointments between people receiving first and second doses, but doing this will create a backlog of people needing a second dose. If you keep giving the first dose without prioritizing people that need the second dose, eventually the backlog of people that need the second dose will snowball. The second dose vaccination date will move very far away from the ideal date and might exceed the due date which will make the first vaccination much less effective.
...
* Hard constraints cannot be broken. If any hard constraint is broken, the plan is infeasible and cannot be executed:
...
image::2021/03/01/vaccination-scheduling.png[]
...
image::2021/03/01/continuous-planning.png[]
...
* https://docs.optaplanner.org/8.3.0.Final/optaplanner-docs/html_single/[OptaPlanner User Guide]
