...
Java executed with the parameters `-Xmx3840M -server -XX:+UseG1GC` and `-Xmx3840M -server -XX:+UseParallelGC` respectively.
In order to run these benchmarks yourself, you need to use the `mvn clean build -DskipTests` command on the project's parent folder and then run the `mvn exec:java` on the optaplanner-examples folder.
...
30 second JVM warm up which is discarded. Each benchmark was run 10 times and the average values were calculated.
