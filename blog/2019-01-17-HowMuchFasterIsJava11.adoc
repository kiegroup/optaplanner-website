...
Java 11 was released some time ago, while https://www.baeldung.com/java-in-2018[majority of developers stay on Java 8].
...
* A stable machine without any other computational demanding processes running was used. Specifically,
2 x Intel® Xeon® CPU E5-2609 0 @ 2.4 GHz (8 cores total) with 31.3 GiB RAM memory, running RHEL 6.

* For both Java versions the benchmark was run with both G1 and Parallel GC, so that we can compare also impact
of garbage collection. Java was executed with following parameters:

  “-Xmx1536M -server -XX:+UseG1GC”  and “-Xmx1536M -server -XX:+UseParallelGC”, respectively.

* JVMs used were Oracle Java 8:
...
* The benchmark uses OptaPlanner version 7.14.0.Final.
...
* The benchmarks measure the number of scores that can be calculated per millisecond. Higher is better. Calculating
...
With Java 11, average improvement is *16.1%* for G1, *4.5%* for Parallel GC. Although Parallel GC is oriented towards
throughput, whereas G1 focuses rather on low-latency garbage collection, the significant improvement of G1 in
Java 11 lead to direct <<table3,comparison of these two garbage collection algorithms>>.
For more information about difference between various GC algorithms, please see e.g.
https://dzone.com/articles/choosing-the-right-gc[choosing-the-right-gc].

It was shown though, that Parallel GC is still the preferred GC for OptaPlanner, as throughput is much more important
for solving optimization problems with OptaPlanner than the latencies introduced by GC.
...
Almost every data set shows improvement on Java 11 over Java 8 using the G1 garbage collector. On average, the is a
16% improvement just by switching to Java 11. Possible reason for such an improvement can be e.g.
...
With Parallel Garbage Collector, the results are less definite as with G1. There is an improvement for some data sets,
...
Although G1 GC shows clear improvement from the Java 8, compared to Parallel GC it’s less beneficial GC strategy
for OptaPlanner on majority of data sets. The only exception is Machine Reassignment, which shows that with
G1 OptaPlanner is able to compute score calculation 83% faster.
This is true for Java 8 too, if you take a close look at <<table1>>.

== Conclusion

Java 11 brings additional improvements, which of course varies across different https://www.optaplanner.org/[OptaPlanner] examples and data sets.
On average, there is 4.5% more performance for free when using Parallel GC and 16.1% more with G1 GC.
Despite the significant improvement  with G1 GC, Parallel GC is still a better option for most of the data sets under test.
