...
Android is not a complete JVM. Some JDK libraries of the `java.beans` package are missing for OptaPlanner to work out of the box.
...
https://play.google.com/store/apps/details?id=org.tomasdavid.vehicleroutingproblem[Download and run this application from the Google Play app store now.]
