...
In fact, the more which people report their benchmarks, the clearer our strengths and weaknesses show.
...
If you use the OptaPlanner Benchmarker toolkit (for your own convenience), simply share the benchmarker HTML report.
...
In short: I like external benchmarks, but dislike competitive benchmarks.
...
A minority of the researchers (with their innovative algorithms in their experimental contraptions and massive time to tweak/build those) still beat us,
...
is a computer and the code. Yet, in practice, the code is usually closed and the raw benchmark data is not accessible.
It's like everyone is scared of sharing the dirty secrets of their code and their benchmarks.

I believe we - the worldwide optimization research community - need to create a *benchmark repository*:
a centralized repository of benchmarks for every use case, for every dataset, for every algorithm, for every implementation version, for every amount of time.
...
We - in OptaPlanner - are well on our way to do exactly that:
