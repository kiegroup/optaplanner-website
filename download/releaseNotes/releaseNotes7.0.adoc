...
For example, we can split up Great Britain into 4 parts and solving those parts separately:
...
There is support to limit the number CPU cores OptaPlanner consumes (potentially starving other processes and thread)
through `runnablePartThreadLimit`. Furthermore, it is also possible to configure the `threadFactoryClass`.

For now, you still need to implement the `SolutionPartitioner` interface,
but we'll add out of the box strategies (such as geo-fencing) for that soon:
