...
image:6.2/nearbySelectionRandomDistribution.png[]
...
VRP with a `@InverseRelationShadowVariable` is now more efficient.
In same cases (incremental score calculation and only change and swap moves), it's up to *`32%` faster* on average.
