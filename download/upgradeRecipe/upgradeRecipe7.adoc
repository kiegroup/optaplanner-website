...
[.upgrade-recipe-minor]
=== Various `Comparators` deprecated for removal
...
We continue to ask users to refer to their `Score`s by their ultimate type, eg. `HardSoftScore` as opposed to `Score`
