...
== From 7.19.0.Final to 7.20.0.Final


[.upgrade-recipe-major]
=== `Solver`: `addEventListener(SolverEventListener)` replaced

The method `Solver.addEventListener(SolverEventListener)` and `Solver.removeEventListener(SolverEventListener)` have been deprecated
in favor of `Solver.addBestSolutionListener(BestSolutionListener)` and `Solver.removeBestSolutionListener(BestSolutionListener)`
to allow other listeners to be added.
...
After in `*.java` (using a function):
[source, java]
----
