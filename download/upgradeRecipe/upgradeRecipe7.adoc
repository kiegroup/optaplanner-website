...
Our new Quarkus and Spring Boot quickstarts provide much better support for annotation processing.
Other users should change their solver configs to enumerate their `@PlanningSolution` and `@PlanningEntity`-annotated classes.
...
scoreExplanation = solverManager.explainScore(solution);
----
