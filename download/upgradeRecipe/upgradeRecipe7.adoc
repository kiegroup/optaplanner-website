...
Various elements of both the solver configuration and benchmark configuration will now log a deprecation warning if
JavaScript expressions are used within.
Users should be replaced with either auto-configuration or with integer constants.
...
Furthermore, to continue to use that deprecated JavaScript configuration together
with JDK 15 or later will have to add the following dependency to their runtime,
