...
=== `@InverseRelationShadowVariable` now supports non-chained planning variables too
...
[.upgrade-recipe-minor]
=== Empty `<constructionHeuristic>`'s default changed
...
[.upgrade-recipe-minor]
=== `solverBenchmarkBluePrintType` `ALL_CONSTRUCTION_HEURISTIC_TYPES` has been renamed
...
[.upgrade-recipe-minor]
=== In real-time planning, `addProblemFactChange()` no longer replaces the `ScoreDirector`
...
[.upgrade-recipe-minor]
=== If your planning solution has a superclass with planner annotations, those will now be ignored
...
[.upgrade-recipe-major]
=== If you have a custom `Move`, its `doMove()` method must now call `scoreDirector.triggerVariableListeners()`
...
[.upgrade-recipe-minor]
=== A custom `Move` must now read any shadow variables it needs first
...
=== All built-in moves that affect chained variables have been greatly simplified
...
[.upgrade-recipe-impl-detail]
=== Constructor of `ChainedChangeMove`, `ChainedSwapMove`, `SubChainChangeMove` and `SubChainSwapMove` changed
...
[.upgrade-recipe-impl-detail]
=== `ChainedSwapMove` 's constructor changed
...
[.upgrade-recipe-impl-detail]
=== Method `InnerScoreDirector.getTrailingEntity()` has been removed
...
[.upgrade-recipe-minor]
=== If you build a `Solver` entirely from API, use `SolverFactory.createEmpty()`
