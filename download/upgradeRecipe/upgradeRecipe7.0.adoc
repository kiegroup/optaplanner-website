...
The `IncrementalScoreCalculator` interface method `calculateScore()` has been changed `calculateScore(initScore)`.
Change the method signature to add the `initScore` and then pass it to the `Score.valueOf()` method.
...
Inside a `ScoreCalculator`, it's must be passed from the `calculateScore()` method (see above).
...
to avoid that a `EasyScoreCalculator` implementation forgets to pass the `initScore` parameter.
