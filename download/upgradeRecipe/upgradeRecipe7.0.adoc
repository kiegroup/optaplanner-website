...
[.upgrade-recipe-major]
=== [line-through]#`EasyScoreCalculator`: `calculateScore()` changed#
...
[.upgrade-recipe-minor]
=== [line-through]#`IncrementalScoreCalculator`: `calculateScore()` changed#
...
[.upgrade-recipe-major.upgrade-recipe-public-api]
=== [line-through]#`Score`: `valueOf(...)` changed and `valueOfInitialized(...)` added#
...
[.upgrade-recipe-minor]
=== [line-through]#Custom initializer: `Score.compareTo()` behaviour changed#
...
[.upgrade-recipe-minor]
=== `CustomPhaseCommand`: method `applyCustomProperties()` moved
...
implement the tagging interface `PropertiesConfigurable` which now has it.

That `PropertiesConfigurable` interface is also supported for other custom classes (such as `SolutionPartitioner`).
...
[source, java]
----
public class MyCustomPhaseCommand extends AbstractCustomPhaseCommand, PropertiesConfigurable {
