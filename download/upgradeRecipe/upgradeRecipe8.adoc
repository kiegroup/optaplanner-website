...
== From 7.43.0.Final or higher to 8.0.0.Beta1
...
[.upgrade-recipe-minor.upgrade-recipe-public-api]
=== `Score`: generic type removed

The interface `Score<Score_ extends Score>` has lost that generic type and is now just `Score`.
Most users use a specific score, such as `HardSoftScore`, and won't be impacted.

Before in `*.java`:

[source,java]
----
public Color determineScoreFieldForeground(Score<?> score) {..}
----
...
[source,java]
----
HardSoftScore score = scoreList.stream()
...
----
HardSoftScore score = scoreList.stream()
        .reduce(HardSoftScore::add).orElse(HardSoftScore.ZERO);
----
