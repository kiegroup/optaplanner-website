= Upgrade recipe 8
:awestruct-description: Upgrade to OptaPlanner 8 from the previous version.
...
[.upgrade-recipe-minor.upgrade-recipe-constraint-match-equals]
=== `ConstraintMatch.compareTo()` inconsistent with `equals()`

The `equals()` override in `ConstraintMatch` has been removed.
As a result, no two different instances of the class will be considered equal.
...
Behavior of `ConstraintMatch` has always been considered an implementation detail.
Users are discouraged from relying on undocumented behavior.