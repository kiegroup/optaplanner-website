...
link:https://www.optaplanner.org/community/getHelp.html[私たちのコミュニティーの公開フォーラム]での質問やlink:https://issues.jboss.org/browse/PLANNER[課題トラッカー]でのバグ報告、機能要望を歓迎します。GitHub への pull 要求を大いに歓迎し、優先的に扱います！ご自身の改善をオープンソース化すれば、それに対して私たちが加える専門的なチェックやさらなる改良をご自身のために役立てることができます。
